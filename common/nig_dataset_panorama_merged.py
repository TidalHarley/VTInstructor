#!/usr/bin/env python3
"""
Panorama 格式的 NIG 数据集（过滤版本）

与原版 nig_dataset_panorama.py 的关键区别：
- 支持加载 filtered_ult.json，自动跳过被过滤的低质量样本
- 训练时只使用通过质量评估的 instruction
...
def load_filtered_episodes(filtered_json: str) -> dict:
    ...
    # 支持两种格式：
    #   格式 1（filter_results.py 输出）: {"metadata": {...}, "episodes": {"1": {"instruction":...}, ...}}
...
class PanoramaFilteredDataset(Dataset):
    ...
    加载 filtered_ult.json，只训练通过质量评估的 episode。
    对每个 episode，使用 filtered_ult.json 中的 instruction（与原始相同）。
    """
    ...
    def __init__(
        ...
        # 可选加载过滤结果（r2rce 通常使用，rxrce 通常不使用）
