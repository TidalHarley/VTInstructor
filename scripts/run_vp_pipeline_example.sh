...
bash "${PROJECT_DIR}/rendering/run_render_r2rce.sh"
bash "${PROJECT_DIR}/rendering/run_render_rxrce.sh"

# Stage 2 — VP overlays + semantic masks (THIS is what training consumes)
...
do
  if [[ -d "$split" ]]; then
    ...
    echo "[Stage 2] skip (missing): ${split}"
  fi
done
