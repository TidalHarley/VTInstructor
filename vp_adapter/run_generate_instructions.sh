...
#
# Wraps vp_adapter/generate_instructions.py with multi-GPU sharding and merges
# the shards back into one records file. No metrics are computed and no
# reference instructions are needed.
#
# Works on plain Stage-1 renders (no VTP, no vp_masks) as well as on Stage-2
# renders; --vp_mode auto decides per split.
#
...
CKPT="${CKPT:-}"
MODEL_DIR="${MODEL_DIR:-}"          # Qwen3-VL snapshot, used as the processor
DATA_DIR="${DATA_DIR:-}"
...
# A checkpoint saved by the SFT trainer usually carries its own processor; point
# MODEL_DIR at the base Qwen3-VL snapshot if yours does not.
...
echo "=========================================="
echo "VTInstructor instruction generation"
echo "  ckpt:    $CKPT"
echo "  data:    $DATA_DIR"
echo "  out:     $OUT_JSON"
