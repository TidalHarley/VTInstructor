...
def _load_vp_mask(mask_path: str, episode_dir: str) -> Optional[np.ndarray]:
    ...
    # Try .npy first (semantic 3-channel)
...
class VPDataset(Dataset):
    ...
    def __init__(
        ...
        # Count how many episodes have VP masks
        mask_count = 0
...
class VPCollator:
    ...
    def __call__(self, batch: list) -> dict:
        ...
        for item in batch:
            ...
            # Collect VP masks: (3, H, W) float tensors, one per image
            for m in vp_masks:
                # m is (H, W, 3) numpy → convert to (3, H, W) tensor
