#!/usr/bin/env python3
"""Benchmark inference for VTInstructor.

Loads a VTMod checkpoint, generates one instruction per trajectory, and writes
a JSON list whose items contain `prediction` plus `references`. That file is
the input to `metrics/evaluate.py` (see `metrics/pred_example.json`).

This script does not compute BLEU / CIDEr / etc. — `eval/run_eval.sh` runs
inference then scoring.
...
Usage:
    python eval/infer.py \
        ...
        --out_json outputs/eval/pred.json
"""
import argparse
import gzip
import json
...
from vtmod.wrapper import attach_vp_adapter, load_vp_modules, generate_with_vp
from vtmod.dataset import (
    _load_vp_mask,
    load_samples,
...
def build_prompt_from_sample(sample: dict, max_frames: int):
    ...
    for i in range(num_frames):
        ...
        if not zero_vp_ablation:
            notify_missing_vp_mask(
                "eval/infer", episode_dir, level="INFO",
...
def main():
    parser = argparse.ArgumentParser(
        description="VTInstructor benchmark inference (writes prediction+references JSON)")
    ...
    parser.add_argument("--r2r_val_json",
                        default=os.path.join(_ROOT, "metrics", "R2R_val_unseen.json"),
    ...
    if not args.out_json:
        ...
        args.out_json = os.path.join(
            _ROOT, "outputs", "eval", f"pred_{ts}.json")

    print("=" * 60)
    print("VTInstructor benchmark inference")
    print("=" * 60)
