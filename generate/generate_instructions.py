...
python generate/generate_instructions.py \
...
from vtmod.dataset import _load_vp_mask, load_samples, shard_files_by_key
...
def main():
    ...
    if not args.out_json:
        ...
        args.out_json = os.path.join(
            _ROOT, "outputs", "augment", f"generated_{ts}.json")
    ...
    if args.vp_mode == "on":
        if not has_masks:
            raise SystemExit(
                ...
                f"(visual_prompt/render_masks.py) first, or use "
                f"--vp_mode auto/off.")
        use_vp = True
