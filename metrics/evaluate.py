#!/usr/bin/env python3
"""Score instruction-generation predictions (no inference).

Reads a JSON list of predictions and prints BLEU / METEOR / ROUGE-L / CIDEr /
SPICE. Each item must look like::

    {"prediction": "...", "references": ["ref 1", "ref 2", "ref 3"]}

``references`` may be a list; a single string field ``reference`` is also
accepted. Extra keys (``trajectory_id``, ``episode_id``, ...) are ignored.
See ``pred_example.json`` in this directory.

Usage::

    python metrics/evaluate.py --pred_json metrics/pred_example.json --skip_spice --skip_meteor
    python metrics/evaluate.py --pred_json outputs/preds.json --out_json outputs/scores.json
"""
import argparse
import json
import os
...
def main():
    ...
    ap.add_argument("--pred_json", required=True,
                    help="JSON list of {prediction, references} (see pred_example.json)")
