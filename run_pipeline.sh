#!/usr/bin/env bash
# VTInstructor — example orchestration of the four stages in order.
#
#     bash run_pipeline.sh
#
# Each stage can also be launched independently (see README).
set -euo pipefail
...
echo "== Stage 1/4: Preprocess / rendering =="
OUT_ROOT="${DATA_ROOT}/R2RCE_visual" bash "${PROJECT_DIR}/preprocess/rendering/run_render_r2rce.sh"
OUT_ROOT="${DATA_ROOT}/RXRCE_visual" bash "${PROJECT_DIR}/preprocess/rendering/run_render_rxrce.sh"

echo "== Stage 2/4: VTP overlay + semantic masks =="
VP_SPLITS=(
...
for split in "${VP_SPLITS[@]}"; do
  echo "[Stage 2] ${split}"
  "${PY}" "${PROJECT_DIR}/visual_prompt/render_masks.py" \
    --data_dir "$split" \
